...
files = files[grepl('_train_', files)]
...
cumIperc_ens <-  lapply(files, function(x) {
  ...
  d = res.train$cumIperc_ens %>% data.frame()
  
  d$date = tmp$Week.start
  d$loc = res.train$loc # id[3] %>% strsplit('\\//') %>% unlist %>% tail(1)
...
cumIperc_ens = melt(cumIperc_ens, id.vars = c('loc','run','date'))
cumIperc_ens = dcast(cumIperc_ens, loc + date ~ run + variable)
