...
## 2/7/22, first training (up to end of projection period), then projection using historical estimates detection rate and ifr
## 2/15/22, specifically account for immune loss for those gained immunity from vaccination
# 3/9/22 tailored for SA
...
EAKF_rproj = function(epi.model=epi.model, num_ens=num_ens,inflat=1.03, 
 ...
 # get prior for week 1
  ...
  for (tt in 1:num_times_i){ # num_times_i
    ...
    if(!(vdate.t %in% tm_rednUpdateS) # this week S can be adjusted
      ...
      if(Spost[tt] - Spost[tt-1] + Itotpost[tt]  > .5/100 * N  # greater than .5% of the population
        ...
        dS.t = xpost['S1',,tt] - xpost['S1',, tt-1] + xpost['newItot1',,tt]  
      ...
      # very small probing on S, when the variant is immune evasive?
      {
        ...
        if(!(vdate.t %in% tm_rednUpdateS) # this week S can be adjusted
          ...
          if(Spost[tt] - Spost[tt-1] + Itotpost[tt] > .8/100 * N &  # greater than .5% of the population
            ...
            SR.perc.imm.t = pmax(SR.perc.imm, pmin(SR.perc.imm * 4, 
                                                   (Spost[tt] - Spost[tt-1] + Itotpost[tt] ) /(1/100 * N) * SR.perc.imm * .8^cntSR.S # (3/100 * N) * SR.perc.imm * .7^cntSR.S
            ))
  ...
  prob.lab_vec = c('median', 'iqr.lwr','iqr.upr','ci95.lwr','ci95.upr', 'ci90.lwr','ci90.upr','ci80.lwr','ci80.upr')
  # get stats instead
  dimnames(xpost)[1] = list(state.names)
  states_stats = NULL
  for(var in state.names){
    tmp = xpost[var,,]
    tmp = tmp %>% apply(2, quantile, prob = prob_vec) %>% t
    colnames(tmp) = prob.lab_vec
  ...
  Rt_stats = Rt_ens %>% apply(1, quantile, prob = prob_vec) %>% t
  colnames(Rt_stats) = prob.lab_vec
  ...
  R0_stats = R0_ens %>% apply(1, quantile, prob = prob_vec) %>% t
  colnames(R0_stats) = prob.lab_vec
  ...
  Rtx_stats = Rtx_ens %>% apply(1, quantile, prob = prob_vec) %>% t
  colnames(Rtx_stats) = prob.lab_vec
  ...
  # print('get the change in tx over diff time periods')
  {
    ...
    # only do those included in this simulation
    tm_variant = tm_variant[date.start <= as.Date(tail(Week.starts,1))]
    ...
    for(iv in 1:nv){
      ...
      if(! grepl('wildtype', tm_variant[iv]$parm)){
        ...
        if(!is.null(stat.dS)){
          ...
          dS.t = data.table(week = numeric(), S0.mean = numeric())
        }
  ...
  if(T){ # don't need this
    ...
    cumIperc_stats = cumIperc %>% apply(1, quantile, prob = prob_vec) %>% t
    colnames(cumIperc_stats) = prob.lab_vec
    ...
    Susceptibility_stats = Susceptibility %>% apply(1, quantile, prob = prob_vec) %>% t
    colnames(Susceptibility_stats) = prob.lab_vec
  ...
  print('start projection')
  {
    ...
    if(length(fcast.mob.t) == num_wk.fast){
      fcast.mob = matrix(da.mob.t[as.Date(date) %in% fcast.wk.starts & data.type == 'business']$mob, nrow = nfcast, ncol = 1)
    } else if(length(fcast.mob.t) < num_wk.fast){
      # incomplete data, use the latest for the missing week
      tmp.mob = fcast.mob.t %>% tail(2) %>% mean
      fcast.mob.full.t = rep(-1, num_wk.fast)
      fcast.mob.full.t[1:length(fcast.mob.t)] = fcast.mob.t
      fcast.mob.full.t[fcast.mob.full.t < 0] = tmp.mob
      fcast.mob = matrix(fcast.mob.full.t, nrow = nfcast, ncol = 1)
    }
    fda.vacc = da.vacc
